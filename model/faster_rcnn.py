...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1.):
        ...
        * :math:`L` is the number of classes excluding the background.
        ...
        h = self.extractor(x)
        # print(h.shape)
        rpn_locs, rpn_scores, rois, roi_indices, anchor,_ = \
            self.rpn(h, img_size, scale)
        # print(roi_indices)
        roi_cls_locs, roi_scores = self.head(
            h, rois, roi_indices)
        #print(roi_cls_locs.shape,roi_scores.shape,rois.shape,roi_indices.shape,'\n\n\n\n\n')
    ...
    def use_preset(self, preset):
        ...
        """
        if preset == 'visualize':
            self.nms_thresh = 0.3    #原来是0.3
            self.score_thresh = 0.7  #原来是0.7
        elif preset == 'evaluate':
            self.nms_thresh = 0.3    #原来是0.3
            self.score_thresh = 0.05   #原来是0.05
        else:
    ...
    @nograd
    def predict(self, imgs, sizes=None, visualize=False, transfer_model=None):
        ...
        if visualize:
            ...
            for img in imgs:
                ...
                if transfer_model !=None:
                    img = transfer_model(torch.Tensor(img).unsqueeze(0).cuda()).squeeze(0)
                # --------
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            # print(roi_cls_loc.shape,roi_scores.shape,rois.shape,'\n\n\n\n')
