...
def visdom_bbox(*args, **kwargs):
    ...
    # plot=fig.plot()
    # fig=plot.get_figure
    # fig.savefig("./image_save/output.png")
    # print(type(fig))
    data = fig4vis(fig)
    #---------------------------------------------------------
    if len(args)==3:
        try:
            os.remove('./image_save/'+str(opt.FILEFOLDER)+'/'
                      +'gt_'+str(opt.IMGINDEX)+'.png')
        except:
            print("没这个文件")
        fig=fig.get_figure()
        fig.savefig('./image_save/'+str(opt.FILEFOLDER)+'/'
                      +'gt_'+str(opt.IMGINDEX)+'.png')
    else:
        try:
            os.remove('./image_save/'+str(opt.FILEFOLDER)+'/'
                      +'pre_'+str(opt.IMGINDEX)+'.png')
        except:
            print("没这个文件")
        fig=fig.get_figure()
        fig.savefig('./image_save/'+str(opt.FILEFOLDER)+'/'
